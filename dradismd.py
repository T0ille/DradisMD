...
from dradis import Dradis # https://github.com/NorthwaveSecurity/dradis-api
...
DRADISMD_VERSION="0.0.1"

SCRIPT_PATH = Path(__file__).parent        # location of dradismd.py
CONFIG_FILE=f"{SCRIPT_PATH}/config.ini"    # location of config file
...
class DradisMD():
    ...
    def list_projects(self, head: int): 
        ...
        for project in sorted_projects:
            ...
            new_row = f"{project['id']}", f"{project['name']}", f"{client}", f"{updated_date} ({time_ago})"
            
            # Get values for custom columns
            for column in custom_columns:
                field = next((field for field in project['custom_fields'] if field["name"] == column), None)
                if field:
    ...
    def import_nodes(self, path: Path ,project_id: int, format: str):
        ...
        for node in self.dradis_nodes:
            ...
            for index, evidence in enumerate(node['evidence']):
                ...
                file.write_text(evidence_content, encoding="utf8", errors="ignore",newline=LINE_RETURN) 
                if format != DRADIS_FORMAT:
                    convert_file(file, DRADIS_FORMAT, format,True)
    ...
    def handle_attachments(self, project_id, node_id, content, file_path) -> str:
        ...
        if attachments:
            ...
            for attachment in attachments:
                ...
                if not any(d['filename'] == attachment_path.name.replace('%20',' ') for d in existing_nodes_attachments):
                    full_attachments_path = Path.resolve(Path.joinpath(file_path, attachment_path.as_posix().replace('%20',' ')))
                ...
                if caption:
                    content = re.sub(re.escape(attachment.get('match')),f"!{dradis_dir}/{attachment_path.name}{caption}!", content)
                else:
                    content = re.sub(re.escape(attachment.get('match')),f"!{dradis_dir}/{attachment_path.name}!", content)
            if already_existed:
                log.debug(f'The following attachments {attachment_path.name} already existed on this Dradis node and were not uploaded.')  
    ...
    def export_evidence(self,project_id,node_id,local_issue_name, evidence_file):
        ...
        if dradis_evidence_content:
            ...
            if not issue_found:
                ...
                dradis_evidence_content = self.handle_attachments(project_id, node_id, dradis_evidence_content, evidence_file.parent)
                ...
                if evidence_id_found:
                    ...
                    try:
                        result = self.api.update_evidence(project_id, node_id, issue_found['id'], evidence_id, dradis_evidence_content)
                        log.debug(result)
                        
                    except:
                        log.warning(f"Could not update {evidence_id}, creating new evidenceID")
                        self.new_evidence(project_id,node_id,  issue_found['id'], dradis_evidence_content, evidence_file, True)
                        
                # New evidence 
                else:
                    log.debug(f"No evidence ID. Creating new")
                    self.new_evidence(project_id,node_id,  issue_found['id'], dradis_evidence_content, evidence_file)   
                                          
    def new_evidence(self,project_id, node_id, issue_id,dradis_evidence_content , evidence, erase_previous_id=False) -> int:
        ...
        evidence.write_text(f"{evidence_content}\n#[EvidenceID]#\n\n{evidence_id}\n", encoding="utf8",newline=LINE_RETURN,errors="ignore") 
...
def convert_files(path,output_format=DRADIS_FORMAT):
    ...
    # Strip Window double quote a the end in of filepath
    path = Path(path.rstrip('"'))
...
def get_pandoc():
    while ( res:=input("Do you want pypandoc to install it for you? [Y/n]").lower() ) not in {"","y", "n"}: pass
    ...
    if not pandoc_installed():
        ...
        if input_format != output_format:
            try:
                ...
                content = re.sub(FIELD_REGEX,f'\g<1>\r\n\r\n\g<2>',content) # Make sure there is always 2 {LINE_RETURN} between #[Field]# and their content
...
def get_item_from_dict_list(dict_list: list, key_to_check: str, matching_string: str) -> dict:
    match = next((dict for dict in dict_list if clean_filename(dict[key_to_check]).lower() == clean_filename(matching_string).lower() ), None)
    return match
...
def test_connection(url):
    ...
    try:
        resp = requests.get(url=url, allow_redirects=True, headers=req_headers, timeout=timeout_time,verify=VERIFY_SSL)
    except:
        return False
        ...
        if work_file.is_file():
            ...
            for index, attachment in enumerate(attachments):
                ...
                if not full_attachments_path.is_file():
                    ...
                    new_filename = renaming_format.replace("%section_initials%",section_initials).replace("%section_title%", title).replace("%foldername%",str(foldername)).replace("%filename%",str(attachments_path.stem)).replace("%count%",str(index+1).zfill(3)).replace("%caption%",clean_filename(attachment.get("caption")))#.replace(" ","_")
                    new_path=f"{full_attachments_path.parent}/{new_filename}{attachments_path.suffix}"
                    log.debug(new_path)
                    content = re.sub(re.escape(attachment.get('match')),f"![{attachment.get('caption')}]({attachments_path.parent}/{new_filename}{attachments_path.suffix}", content)#![{caption}]({new_path})
...
def print_help():
    ...
    help_table.add_row("list\n    [dim][--head][/dim]","\n[dim]<number>[/dim]","List projects with their IDs in last updated order\n[dim]Show only last X projects[/dim]")
    help_table.add_row("get\n   [dim][--format][/dim]","<project_id> <destination_folder>\n[dim]<format>","Import a project from Dradis to local folder\n[dim]Format to convert to when importing[/dim]")

    help_table.add_row("update","<project_id> <source>","Export your local projects files to Dradis")
    help_table.add_row("convert","<folder|file> <format>",f"Convert a project file (markup format) or all project files in a folder to another supported format {list(SUPPORTED_FORMAT.keys())}")
    help_table.add_row("rename","<input_file> <attachments_folder>","Rename all attachments in a file with correct Dradis path and rename with format from config file")

    console.print(help_table)
    console.print("[highlight]Example of use:[/highlight]\npython dradismd.py [args]list[/args]\npython dradismd.py [args]get 47 /workfolder/pentests [dim]--format markdown[/dim][/args]",style=None)
...
def arg_parser():
    ...
    parser_list.add_argument("--head", action="store", type = int)
...
def main():
    console.print(f" > Dradis[bold]MD[/bold] [grey]v{DRADISMD_VERSION}[/grey]",style="title",highlight=False,)
    args = arg_parser()
    if args.action in ["list","get","update"]:
